...
typedef struct wasm_thread_data {
  ...
  wasm_thread_wait_node wait_node;
...
static void
thread_data_destroy(wasm_thread_data *thread_data)
{
...
static void
ws_thread_wrapper(void *arg)
{
  wasm_thread_data *thread_data = arg;
                           ...
                           void *arg, unsigned int stack_size, int prio)
{
  ...
  memset(thread_data, 0, sizeof(wasm_thread_data));
...
{
  (void)value_ptr;
  wasm_thread_data *thread_data, *curr_thread_data;
  ...
  /* Get thread data */
  thread_data = (wasm_thread_data*)thread;
...
int
ws_tls_put(unsigned idx, void * tls)
{
  wasm_thread_data *thread_data;
...
{
  wasm_thread_wait_node *node = &thread_data_current()->wait_node;
